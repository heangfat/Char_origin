...
共鈕件 = ["一●乛","囗〇","冈网","尞㶫","厂丆","上丄","下丅","凵𠙴","朩汖","兕𤉡","中𠁩","且𠀃","叀𤰔","卣㔽","乂㐅","乙𠃉","几𠘧","冉冄","田龱甶","垂𠂹","壼𡆵","井丼","卩㔾","角甪","𫢉佘","于亐","皃㫐","車𠦴𨏖"]+ 挪用歸并件# ,"眉𦭝"
...
class 飭碼表():
	...
	def 統計(self, 圖=False):
		非正解之構件 = '';非文 = []
		for g in self.解之名[1:]:	非正解之構件 += self.碼表['迩原'+g].astype(str).str.cat(sep='')
		非正解之部件 = ''.join(set(非正解之構件))
		非正解之部件 = re.sub('['+(''.join(冓.keys()))+']','',非正解之部件)
		for 非正解部件 in 非正解之部件:# 查非文
			其正解 = self.碼表.loc[self.碼表["漢字"]==非正解部件, "迩原正解"]
			if len(其正解) > 0 and 非正解部件 != 其正解.item():
				非文.append((非正解部件, 非正解之構件.count(非正解部件)))
		所有構件 = self.碼表['迩原正解'].str.cat(sep='') + 非正解之構件
