...
挪用歸并件 = ["戶戸","辛⾟"]# 倒〬三角頭飾
挪用反倒轉件 = ["舟⾈","水⽔","弓⼸","矢⽮","臣⾂"]# 雋之弓、受之舟、臦之反臣、益之水、射之矢
歸并件 = ["一●乛","囗〇","冈网","尞㶫","厂丆","上丄","下丅","凵𠙴","朩汖","兕𤉡","中𠁩","卣㔽","乂㐅","乙𠃉","几𠘧","冉冄","田龱","首𦣻","子㜽","垂𠂹","壼𡆵","井丼", "羽习"]+ 挪用歸并件
反倒轉件 = ["止𣥂夂㐄","爪仉𠂇又","永𠂢","人匕𠤎𠂈","身㐆","大屰","卩㔿","孑孓","了巳","欠旡","口亼","目罒","矢𢆉","冉𠀎","司后","皿亚","彳亍","戶𠃛","毛氺","上下","亅𠄌","一丨"]+ 挪用反倒轉件
...
def 依構件排序(構件列):
	...
	return 序
def 依備註排序(註列):
	訛宂次 = {"異體":1, "偏旁":2, "倭":3, "国字":3, "簡体":3, "訛":5}
	序 = []
	for 註 in 註列:
		if 註 in 訛宂次.keys():
			序.append(訛宂次[註])
